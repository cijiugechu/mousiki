...
- `test_opus_projection.c` still needs the ambisonics projection encoder/decoder and the projection decode/encode harnesses. The `mapping_matrix_*` helpers now mirror the reference pointer/stride semantics in `src/mapping_matrix.rs`, and `src/projection.rs` covers ambisonics channel validation/matrix selection and demixing export helpers, but the mapping-family-3 multistream front-ends remain unported (encode/decode shims, CTLs for mixing/demixing and matrix gain export, and Ogg header plumbing).
