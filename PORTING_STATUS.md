...
- `test_opus_projection.c` needs the ambisonics projection encoder/decoder and the `mapping_matrix_*` helpers from `src/mapping_matrix.{c,h}`; no projection API or matrix math exists on the Rust side yet.
