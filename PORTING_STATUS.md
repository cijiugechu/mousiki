...
- `opus-c/celt/tests/test_unit_dft.c` — complex FFT accuracy harness; **Ported** as the `fft_matches_reference_across_sizes` unit test in `src/celt/kiss_fft.rs`, exercising the bit-reversal/stride planning and normalised forward transform.
...
- `opus-c/celt/tests/test_unit_{mathops,entropy,mdct,cwrs32,rotation}.c` — CELT unit tests are not yet represented in Rust.
...
- `test_opus_api.c` exercises the public encoder/decoder, CTLs, repacketizer, packet pad/unpad helpers, and the multistream front-ends. The Rust tree lacks the `opus_decode{,_float,_native}` path, the `OpusEncoder`/`opus_encode` wrappers, and the `opus_multistream_*` encode/decode shims. The repacketizer plus `opus_packet_pad/unpad` and multistream pad/unpad helpers now exist, but packet extensions are still stubbed.
...
- `test_opus_extensions.c` exercises the packet extension helpers (`opus_packet_extensions_*`) and repacketizer extension passthrough; Rust still lacks the extension encode/decode logic, so repacketizer extension handling is stubbed.
- `test_opus_projection.c` still needs the ambisonics projection encoder/decoder and the projection decode/encode harnesses. The `mapping_matrix_*` helpers now mirror the reference pointer/stride semantics in `src/mapping_matrix.rs`, and `src/projection.rs` covers ambisonics channel validation/matrix selection plus demixing export helpers (size/gain/matrix), but the mapping-family-3 multistream front-ends remain unported (encode/decode shims and Ogg header plumbing).
...
- `celt/tests/test_unit_{mathops,entropy,types,mdct,cwrs32,rotation}.c` validate the CELT primitives against reference vectors. Although many primitives are already ported (range coder, Laplace models, CWRS, MDCT, rotation), there is no Rust harness mirroring these vector checks. The scalar FFT path now includes the allocation/bit-reversal layer and DFT harness in `src/celt/kiss_fft.rs`, and the ARM/x86 RTCD tables now live in `src/celt/arm_celt_map.rs` and `src/celt/x86_celt_map.rs` (still selecting the scalar helpers while `OPUS_ARCHMASK` is zero), but SIMD back-ends and runtime detection remain unported.
